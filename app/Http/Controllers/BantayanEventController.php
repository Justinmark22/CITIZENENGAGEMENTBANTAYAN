...
class BantayanEventController extends Controller
{
    public function index()
{
    $now = now();

    $events = Event::where('location', 'Bantayan')
                   ->whereDate('event_date', '>=', $now->toDateString())
                   ->orderBy('event_date')
                   ->get();

    return view('bantayan.events', compact('events'));
}
...
public function forward(Request $request, $id)
{
   ...
   ForwardedEvent::create([
    ...
    {
        $event = Event::findOrFail($id);
        return view('bantayan.events_edit', compact('event'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('bantayan.events')->with('success', 'Event updated successfully.');
    }
