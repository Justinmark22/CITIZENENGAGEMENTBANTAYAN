...
<nav class="w-full bg-white/90 backdrop-blur-md border-b shadow-sm px-4 md:px-6 py-3 flex items-center justify-between sticky top-0 z-50">
  ...
  <a href="#" class="flex items-center gap-3">
    <img src="<?php echo e(asset('images/citizen.png')); ?>" alt="Logo" class="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border border-gray-200 shadow-sm">
...
<button onclick="toggleDropdown('alertsDropdown'); clearBadge();" 
  ...
  <?php if($mddrmoAcceptedReports->isNotEmpty() || $mddrmoOngoingReports->isNotEmpty() || $mddrmoResolvedReports->isNotEmpty()): ?>
    <span id="alertsBadge" 
      ...
      <?php echo e($mddrmoAcceptedReports->count() + $mddrmoOngoingReports->count() + $mddrmoResolvedReports->count()); ?>
  ...
  <?php endif; ?>
      ...
      <div id="alertsDropdown" class="hidden absolute right-0 mt-2 w-80 md:w-96 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden z-50">
        ...
        <div class="max-h-96 overflow-y-auto">
          ...
          <?php $__empty_1 = true; $__currentLoopData = $alerts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alert): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div onclick="showAlertModal(<?php echo e($alert->id); ?>, '<?php echo e($alert->title); ?>', '<?php echo e($alert->message); ?>')" 
              ...
              <div class="flex-1">
                <p class="text-gray-800 text-sm font-medium"><?php echo e($alert->title); ?></p>
                <p class="text-gray-500 text-xs mt-1"><?php echo e($alert->message); ?></p>
              </div>
            </div>
          ...
          <?php $__currentLoopData = ['Resolved Reports' => ['color'=>'purple','data'=>[$mddrmoResolvedReports,$wasteResolvedReports]],
                    'Ongoing Reports' => ['color'=>'blue','data'=>[$mddrmoOngoingReports,$wasteOngoingReports]],
                    'Accepted Reports' => ['color'=>'green','data'=>[$mddrmoAcceptedReports,$wasteAcceptedReports]]]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group => $groupData): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php $__currentLoopData = $groupData['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reports): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <?php $__currentLoopData = $reports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div onclick="openReportModal(<?php echo e($report->id); ?>, '<?php echo e($report->title); ?>', '<?php echo e($report->status); ?>', '<?php echo e($report->forwarded_to); ?>')" 
                     class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 cursor-pointer transition bg-<?php echo e($groupData['color']); ?>-50 rounded-md m-2">
                  <div class="flex-shrink-0 w-8 h-8 bg-<?php echo e($groupData['color']); ?>-400 text-white rounded-full flex items-center justify-center">
                    <i data-lucide="check-circle" class="w-4 h-4"></i>
                  </div>
                  <div class="flex-1">
                    <p class="text-<?php echo e($groupData['color']); ?>-700 text-sm font-medium"><?php echo e($report->status); ?></p>
                    <p class="text-gray-600 text-xs mt-1">
                      Your report "<span class="font-medium"><?php echo e($report->title); ?></span>" 
                      <?php echo e($group == 'Resolved Reports' ? 'was resolved by' : ($group == 'Ongoing Reports' ? 'is being handled by' : 'was forwarded to')); ?>

                      <span class="font-semibold"><?php echo e($report->forwarded_to); ?></span>.
                    </p>
                  </div>
                </div>
    ...
    <!-- 📩 Feedback -->
    <a href="<?php echo e(route('feedback.page')); ?>" class="flex items-center gap-1 text-gray-700 hover:text-green-700 transition">
    ...
    <!-- 💬 Support -->
    <a href="<?php echo e(route('contact.support.page')); ?>" class="flex items-center gap-1 text-gray-700 hover:text-green-700 transition">
...
<div class="relative">
  ...
  >
    ...
    <div class="hidden sm:block px-3 py-2 bg-gray-50 text-xs sm:text-sm text-center leading-tight">
      <p class="font-semibold truncate"><?php echo e(Auth::user()->name ?? 'Guest'); ?></p>
      <p class="text-gray-500 truncate"><?php echo e(Auth::user()->email ?? 'No Email'); ?></p>
      <p class="text-gray-400 truncate"><?php echo e(Auth::user()->location ?? 'No Location'); ?></p>
    </div>
  ...
  <div class="px-6 md:px-8 flex flex-col lg:flex-row gap-10 items-start">
    ...
    <div class="lg:w-1/2 max-w-xl flex flex-col justify-center">
      <h1 class="text-3xl md:text-5xl font-extrabold mb-3 md:mb-4 leading-tight text-gray-900">
        Welcome, <span class="text-green-700"><?php echo e(Auth::user()->name ?? 'Guest'); ?></span>
      </h1>
...
<?php $__empty_1 = true; $__currentLoopData = $mddrmoResolvedReports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
  <?php $isNew = \Carbon\Carbon::parse($report->updated_at)->gt(now()->subDay()); ?>
    ...
    <div class="flex justify-between items-start gap-3">
      <div class="flex flex-col gap-2">
        <strong class="text-gray-800 text-md animate-pulse">
          Hello <?php echo e($report->user ? $report->user->name : 'Citizen'); ?> (ID: <?php echo e($report->user_id); ?>)
        </strong>

        <p class="text-gray-700 text-sm">
          Your report titled <em>"<?php echo e($report->title); ?>"</em> in the category 
          <span class="font-semibold text-green-600"><?php echo e($report->category); ?></span> 
          has been successfully resolved.
        </p>

        <div class="text-xs text-gray-500 mt-2 space-y-1 flex flex-col">
          <div class="flex items-center gap-1">
            <i data-lucide="calendar" class="w-4 h-4 text-green-600"></i> Submitted: <?php echo e($report->created_at->format('M d, Y h:i A')); ?>

          </div>
          <div class="flex items-center gap-1">
            <i data-lucide="check-circle" class="w-4 h-4 text-green-600"></i> Resolved: <?php echo e($report->updated_at->format('M d, Y h:i A')); ?>

          </div>
          <div class="flex items-center gap-1">
            <i data-lucide="hash" class="w-4 h-4 text-gray-400"></i> Report ID: <span class="font-mono"><?php echo e($report->id); ?></span>
            <button onclick="navigator.clipboard.writeText('<?php echo e($report->id); ?>')" class="ml-2 text-xs text-green-600 hover:text-green-700" title="Copy ID">
      ...
      <div class="flex flex-col items-end gap-2">
        ...
        <span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-600 flex items-center gap-1">
          <i data-lucide="tag" class="w-3 h-3"></i> <?php echo e($report->category); ?>
...
<button onclick="window.location='<?php echo e(route('feedback.page', ['report' => $report->id])); ?>'" 
...
<div class="bg-white rounded-3xl shadow-xl p-6 h-full flex flex-col">
  ...
  <div class="overflow-auto space-y-5 flex-1" style="max-height: 480px;">
    ...
    <?php $__empty_1 = true; $__currentLoopData = $wasteResolvedReports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <?php $isNew = \Carbon\Carbon::parse($report->updated_at)->gt(now()->subDay()); ?>
      <div class="relative group bg-gradient-to-r from-yellow-50 to-yellow-100 border-l-4 border-yellow-500 rounded-2xl p-5 shadow-md hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-500 overflow-hidden animate-fadeIn">
        ...
        <div class="flex justify-between items-start gap-3">
          <div class="flex flex-col gap-2">
            <strong class="text-gray-800 text-md animate-pulse">
              Hello <?php echo e($report->user ? $report->user->name : 'Citizen'); ?>,
            </strong>
            <p class="text-gray-700 text-sm">
              Your report titled <em>"<?php echo e($report->title); ?>"</em> in the category 
              <span class="font-semibold text-yellow-600"><?php echo e($report->category); ?></span> 
              has been successfully resolved by <strong>Waste Management</strong>.
            </p>
            <div class="text-xs text-gray-500 mt-2 space-y-1 flex flex-col">
              <div class="flex items-center gap-1">
                <i data-lucide="calendar" class="w-4 h-4 text-yellow-600"></i> Submitted: <?php echo e($report->created_at->format('M d, Y h:i A')); ?>

              </div>
              <div class="flex items-center gap-1">
                <i data-lucide="check-circle" class="w-4 h-4 text-yellow-500"></i> Resolved: <?php echo e($report->updated_at->format('M d, Y h:i A')); ?>

              </div>
              <div class="flex items-center gap-1">
                <i data-lucide="hash" class="w-4 h-4 text-gray-400"></i> Report ID: <span class="font-mono"><?php echo e($report->id); ?></span>
                <button onclick="navigator.clipboard.writeText('<?php echo e($report->id); ?>')" class="ml-2 text-xs text-yellow-500 hover:text-yellow-700" title="Copy ID">
                  <i data-lucide="clipboard" class="w-4 h-4"></i>
                </button>
              </div>
            </div>
...
<div id="reportModal" class="hidden fixed inset-0 bg-black/50 z-50 items-center justify-center p-4">
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 animate-fadeIn">
    ...
    <form id="submitConcernForm" action="<?php echo e(route('reports.store')); ?>" method="POST" enctype="multipart/form-data">
      <?php echo csrf_field(); ?>
      <div class="mb-3">
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/dashboard/bantayan.blade.php ENDPATH**/ ?>