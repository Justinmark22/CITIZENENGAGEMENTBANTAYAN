...
<body class="bg-gray-900 flex items-center justify-center min-h-screen px-4">
  ...
  <div class="w-full max-w-4xl bg-gray-800 rounded-2xl shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-2">
    ...
    <div class="flex flex-col justify-center items-center p-8 lg:p-12 text-center lg:text-left animate-fadeInUp">
      <img src="<?php echo e(asset('images/citizen.png')); ?>" alt="Citizen Logo"
    ...
    <div class="p-8 lg:p-12 animate-fadeInUp">
      ...
      <form id="loginForm" method="POST" action="<?php echo e(route('login.submit')); ?>" class="space-y-4">
        <?php echo csrf_field(); ?>
        ...
        <fieldset id="loginFieldset">
          ...
          <div class="flex flex-col md:flex-row md:justify-between md:items-center text-sm text-gray-300 mt-2">
            ...
            <a href="<?php echo e(route('password.request')); ?>" class="text-indigo-400 hover:underline">Forgot Password?</a>
          </div>
        </fieldset>
        ...
        <p class="text-center text-gray-400 text-sm mt-2">
          Don't have an account? <a href="<?php echo e(route('register')); ?>" class="text-indigo-400 hover:underline">Register here</a>
        </p>
      </form>
    </div>
  </div>
...
const loginErrors = <?php echo json_encode($errors->any(), 15, 512) ?>;
const loggedInStatus = <?php echo json_encode(session('status') === 'logged_in', 15, 512) ?>;
...
grecaptcha.ready(function() {
  grecaptcha.execute('<?php echo e(env("RECAPTCHA_SITE_KEY")); ?>', { action: 'login' }).then(function(token) {
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/login.blade.php ENDPATH**/ ?>