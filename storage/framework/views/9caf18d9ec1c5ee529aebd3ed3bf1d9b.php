...
<body class="bg-gray-50 font-sans min-h-screen flex">
  ...
  <aside class="w-64 bg-white shadow-lg flex flex-col">
    ...
    <nav class="mt-6 flex-1 space-y-2">
      ...
      <a href="<?php echo e(route('water.reports-bantayan')); ?>" class="flex items-center px-6 py-3 text-gray-700 hover:bg-blue-100 rounded-lg transition">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  <main class="flex-1 p-6 overflow-y-auto">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
      <div class="bg-white p-4 rounded-xl shadow flex flex-col items-center hover:shadow-lg transition">
        ...
        <span id="count-pending" class="text-xl font-bold text-yellow-600"><?php echo e($reports->where('status','Pending')->count()); ?></span>
      </div>
      <div class="bg-white p-4 rounded-xl shadow flex flex-col items-center hover:shadow-lg transition">
        <span class="text-sm text-gray-500">Accepted</span>
        <span id="count-accepted" class="text-xl font-bold text-blue-600"><?php echo e($reports->where('status','Accepted')->count()); ?></span>
      </div>
      <div class="bg-white p-4 rounded-xl shadow flex flex-col items-center hover:shadow-lg transition">
        <span class="text-sm text-gray-500">Ongoing</span>
        <span id="count-ongoing" class="text-xl font-bold text-orange-600"><?php echo e($reports->where('status','Ongoing')->count()); ?></span>
      </div>
      <div class="bg-white p-4 rounded-xl shadow flex flex-col items-center hover:shadow-lg transition">
        <span class="text-sm text-gray-500">Resolved</span>
        <span id="count-resolved" class="text-xl font-bold text-green-600"><?php echo e($reports->where('status','Resolved')->count()); ?></span>
      </div>
    </div>
    ...
    <div class="space-y-6" id="reports-container">
      ...
      <div x-data="{ open: false }" id="report-<?php echo e($report->id); ?>"
        ...
        <div @click="open = !open" class="cursor-pointer px-6 py-4 flex justify-between items-center hover:bg-gray-50 transition">
          <div>
            <h2 class="text-lg font-semibold text-gray-800"><?php echo e($report->title); ?></h2>
            <p class="text-sm text-gray-500 mt-1">
              <?php echo e($report->category); ?> • ID: <?php echo e($report->id); ?>

              <span class="ml-2">By: <?php echo e($report->user?->name ?? 'N/A'); ?></span>
            </p>
          </div>
          <div class="flex items-center gap-3">
            <!-- Status Badge -->
            <span class="status-badge px-3 py-1 text-xs font-semibold rounded-full 
              <?php echo e(strtolower($report->status)=='pending'?'bg-yellow-400 text-yellow-900':(strtolower($report->status)=='ongoing'?'bg-orange-400 text-orange-900':'bg-blue-500 text-white')); ?>">
            ...
            <span class="px-2 py-0.5 text-xs font-semibold rounded-full
              <?php echo e($report->type=='rerouted'?'bg-purple-500 text-white':'bg-green-500 text-white'); ?>">
              <?php echo e(ucfirst($report->type ?? 'Forwarded')); ?>
        ...
        <div x-show="open" x-transition class="px-6 py-4 border-t border-gray-100 space-y-4 text-gray-700 bg-gray-50/50 rounded-b-xl">
          ...
          <span><strong>Created:</strong> <?php echo e($report->created_at ? \Carbon\Carbon::parse($report->created_at)->format('M d, Y h:i A') : 'N/A'); ?></span>

          </div>

          <?php if($report->photo): ?>
          <img src="<?php echo e(asset('storage/'.$report->photo)); ?>" class="rounded-lg w-full md:w-2/3 lg:w-1/2 h-48 object-cover border border-gray-200 shadow-sm">
          <?php endif; ?>

          <div class="flex flex-wrap justify-end gap-2 mt-4">
            <button onclick="updateStatus('<?php echo e($report->id); ?>','Accepted',{},this)" class="flex items-center gap-1 px-4 py-1 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600 transition">
              <i data-lucide="check-circle" class="w-4 h-4"></i> Accept
            </button>
            <button onclick="updateStatus('<?php echo e($report->id); ?>','Ongoing',{},this)" class="flex items-center gap-1 px-4 py-1 bg-yellow-500 text-white rounded-lg shadow hover:bg-yellow-600 transition">
              <i data-lucide="loader" class="w-4 h-4 animate-spin"></i> Ongoing
            </button>
            <button onclick="updateStatus('<?php echo e($report->id); ?>','Resolved',{},this)" class="flex items-center gap-1 px-4 py-1 bg-green-500 text-white rounded-lg shadow hover:bg-green-600 transition">
              <i data-lucide="check" class="w-4 h-4"></i> Resolved
            </button>

            <button onclick="rerouteReport('<?php echo e($report->id); ?>')" 
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/water/reports-bantayan.blade.php ENDPATH**/ ?>