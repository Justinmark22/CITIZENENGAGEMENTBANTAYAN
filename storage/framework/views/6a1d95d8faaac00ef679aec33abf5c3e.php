...
<aside id="sidebar" class="fixed top-0 left-0 w-64 h-full bg-gradient-to-b from-blue-900 to-blue-800 text-white p-6 transform -translate-x-full lg:translate-x-0 z-50">
  ...
  <nav class="flex flex-col gap-2 text-sm">
    ...
    <a href="<?php echo e(route('dashboard.admin')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-speedometer2"></i> Dashboard</a>
    <a href="<?php echo e(route('admin.analytics')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-graph-up-arrow"></i> Analytics</a>

    <p class="uppercase text-xs opacity-70 mt-3">User Management</p>
    <a href="<?php echo e(route('admin.users.index')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-people"></i> Users</a>
    <a href="<?php echo e(route('admin.municipal.index')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-person-badge"></i> Municipal Admins</a>

    <p class="uppercase text-xs opacity-70 mt-3">Content</p>
    <a href="<?php echo e(route('admin.announcements.index')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-megaphone"></i> Announce</a>
    <a href="<?php echo e(route('admin.reports.index')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-file-text"></i> Reports</a>
    <a href="<?php echo e(route('admin.updates.create')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-plus-square"></i> Updates</a>
    <a href="<?php echo e(route('admin.events.create')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-calendar-event"></i> Events</a>
    <a href="<?php echo e(route('admin.engagements.index')); ?>" class="flex items-center gap-2 p-2 rounded hover:bg-white/10"><i class="bi bi-people-fill"></i> Engagement</a>
  </nav>
</aside>
...
<div class="lg:ml-64 p-3 sm:p-4 lg:p-8">
  ...
  <a href="<?php echo e(route('view.bantayan')); ?>" class="relative font-semibold">
    Bantayan 
  ...
  <a href="<?php echo e(route('view.madridejos')); ?>" class="relative font-semibold">
    Madridejos 
  ...
  <a href="<?php echo e(route('view.santafe')); ?>" class="relative font-semibold">
    Sta. Fe 
    ...
    <div class="relative">
      <div onclick="toggleDropdown()" class="flex items-center gap-2 cursor-pointer">
        <img src="<?php echo e(asset('images/citizen.png')); ?>" class="w-9 h-9 rounded-full border">
      ...
      <div id="dropdownMenu" class="hidden absolute right-0 bg-white shadow rounded mt-2 w-32">
        <form method="POST" action="<?php echo e(route('logout')); ?>" id="logoutForm"><?php echo csrf_field(); ?>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
  ...
  <?php $__currentLoopData = $stats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="stats-card bg-white rounded-2xl p-5 shadow-lg text-center hover:shadow-xl transition">
      <p class="text-sm text-gray-800 font-bold"><?php echo e($stat['label']); ?></p>
      <p class="text-xl sm:text-2xl font-normal text-<?php echo e($stat['color']); ?>-600"><?php echo e($stat['value']); ?></p>
    </div>
...
<div id="adminModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200">
    ...
    <!-- Body -->
    <div class="p-6">
      <form method="POST" action="<?php echo e(route('admins.store')); ?>" class="space-y-5">
        <?php echo csrf_field(); ?>
...
<div id="alertModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <div class="bg-white rounded-xl w-full max-w-lg p-6">
    ...
    <form id="alertForm" method="POST" action="<?php echo e(route('admin.alerts.store')); ?>">
      <?php echo csrf_field(); ?>
      <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ✅ Charts
  const reportDays = <?php echo json_encode($reportDays); ?>;
  const reportCounts = <?php echo json_encode($reportCounts); ?>;
  const pendingReports = <?php echo e($pendingReportsCount); ?>;
  const resolvedReports = <?php echo e($resolvedReports); ?>;
  const usersSantaFe = <?php echo e($santaFeUsers ?? 0); ?>;
  const usersBantayan = <?php echo e($bantayanUsers ?? 0); ?>;
  const usersMadridejos = <?php echo e($madridejosUsers ?? 0); ?>;
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/dashboard/admin.blade.php ENDPATH**/ ?>