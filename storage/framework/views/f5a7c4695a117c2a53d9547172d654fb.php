...
<nav class="w-full bg-white/90 backdrop-blur-md border-b shadow-sm px-4 md:px-6 py-3 sticky top-0 z-50" x-data="{ open: false, userOpen: false }">
    ...
    <div class="hidden md:flex items-center gap-4 md:gap-5">
      ...
      <div class="relative" @click.away="userOpen = false">
        <button @click="userOpen = !userOpen" class="flex items-center gap-2 text-gray-700 hover:text-green-700 transition">
          ...
          <span class="hidden md:inline font-medium"><?php echo e(Auth::user()->name ?? 'Guest'); ?></span>
  ...
  <div x-show="open" @click.outside="open = false" class="md:hidden mt-2 space-y-2 px-2" x-cloak>
    <a href="<?php echo e(route('feedback.page')); ?>" class="block px-4 py-2 rounded-lg text-gray-700 hover:bg-green-100 transition">Feedback</a>
    <a href="<?php echo e(route('contact.support.page')); ?>" class="block px-4 py-2 rounded-lg text-gray-700 hover:bg-green-100 transition">Support</a>
    ...
    <div class="w-full lg:w-1/2 max-w-md">
      <div class="relative group bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-600 rounded-3xl p-6 shadow-md hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-500 overflow-hidden animate__animated animate__fadeInUp">
        ...
        <?php if(session('success')): ?>
          <div class="bg-green-100 border-l-4 border-green-600 p-3 rounded text-green-800 text-sm mb-2"><?php echo e(session('success')); ?></div>
        <?php elseif(session('error')): ?>
          <div class="bg-red-100 border-l-4 border-red-600 p-3 rounded text-red-800 text-sm mb-2"><?php echo e(session('error')); ?></div>
        <?php endif; ?>
        ...
        <form method="POST" action="<?php echo e(route('contact.send')); ?>" class="space-y-4">
          ...
          <div>
            ...
            <input type="text" name="name" value="<?php echo e(Auth::user()->name ?? ''); ?>" class="w-full px-4 py-2 rounded-xl border border-green-300 focus:ring-2 focus:ring-green-500 focus:outline-none bg-white/80 text-sm" required>
          </div>
          <div>
            <label class="block text-green-800 font-semibold mb-1 text-sm">Email</label>
            <input type="email" name="email" value="<?php echo e(Auth::user()->email ?? ''); ?>" class="w-full px-4 py-2 rounded-xl border border-green-300 focus:ring-2 focus:ring-green-500 focus:outline-none bg-white/80 text-sm" required>
          </div>
          <div>
           <div>
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/contact_support.blade.php ENDPATH**/ ?>