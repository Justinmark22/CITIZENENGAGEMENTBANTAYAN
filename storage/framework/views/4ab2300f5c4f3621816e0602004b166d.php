...
<!-- Logo Section -->
 <div class="text-center mb-4">
  <img src="<?php echo e(asset('images/santafe.png')); ?>" alt="Santa Fe Logo" class="img-fluid rounded-circle" style="max-height: 80px; width: 80px; object-fit: cover;">
</div>

  <!-- Menu Links -->
  <a href="<?php echo e(route('dashboard.bantayanadmin')); ?>">
 ...
 <a href="<?php echo e(route('bantayan.updates')); ?>">
...
<div class="main">
  <div class="card-box mb-4">
    ...
    <!-- Filters -->
   <form method="GET" action="<?php echo e(route('bantayan.reports')); ?>" class="row g-2 mb-4">
  <div class="col-md-3">
...
<div class="flex-grow-1 pe-3">
  <h6 class="fw-bold text-dark mb-1 cursor-pointer d-flex align-items-center gap-2"
      ...
      data-location="<?php echo e($report->location); ?>"
      data-status="<?php echo e($report->status); ?>"
      data-date="<?php echo e($report->created_at->format('M d, Y H:i')); ?>"
     data-photo="<?php echo e($report->photo ? asset('storage/'.$report->photo) : ''); ?>">
        ...
        <p class="text-muted small mb-2"><?php echo e(Str::limit($report->description, 120)); ?></p>

        <!-- Report Meta Info -->
        <div class="text-muted small d-flex align-items-center flex-wrap gap-3 mb-1">
          <span><i data-lucide="map-pin" class="me-1"></i> <?php echo e($report->location); ?></span>
          <span><i data-lucide="clock" class="me-1"></i> <?php echo e($report->created_at->format('M d, Y H:i')); ?></span>
        </div>
<div class="text-muted small d-flex align-items-center flex-wrap gap-3">
  <span><i data-lucide="user" class="me-1"></i> <?php echo e($report->user->name ?? 'Anonymous'); ?></span>
  <span><i data-lucide="mail" class="me-1"></i> <?php echo e($report->user->email ?? 'No Email'); ?></span>
</div>
    ...
    <?php echo e($reports->withQueryString()->links()); ?>
      ...
      <div class="text-end" style="z-index: 1050;">
        ...
        <span class="badge rounded-pill px-3 py-2 fw-semibold text-uppercase shadow-sm mb-2 d-inline-block
          <?php if($report->status === 'Resolved'): ?> bg-gradient-success 
          <?php elseif($report->status === 'Rejected'): ?> bg-gradient-danger 
          <?php elseif($report->status === 'Ongoing'): ?> bg-gradient-info 
          <?php else: ?> bg-gradient-warning 
          <?php endif; ?>">
        ...
        <div class="dropup">
          ...
          <ul class="dropdown-menu dropdown-menu-end shadow rounded-3" style="z-index: 2000;">
            <?php if($report->status === 'Resolved'): ?>
              <li>
                <a href="<?php echo e(route('bantayan.export', $report->id)); ?>" class="dropdown-item d-flex align-items-center gap-2 text-primary">
                  <i data-lucide="download"></i> Export PDF
                </a>
              </li>
            <?php else: ?>
              <li>
                <form method="POST" action="<?php echo e(route('bantayan.reports.update', $report->id)); ?>">
      ...
      <div class="px-5 pt-4 pb-3 border-bottom" style="border-color: rgba(0,0,0,0.08);">
        <div class="d-flex flex-wrap gap-4 text-muted small">
          <div class="d-flex align-items-center gap-2">
            ...
            <span id="modalReportName" class="fw-semibold text-dark"><?php echo e($report->user->name ?? 'Anonymous'); ?></span>
          </div>
          <div class="d-flex align-items-center gap-2">
            <i data-lucide="mail" class="text-primary"></i>
            <span id="modalReportEmail" class="fw-semibold text-dark"><?php echo e($report->user->email ?? 'No Email'); ?></span>
          </div>
          <div class="d-flex align-items-center gap-2">
    <i data-lucide="hash" class="text-primary"></i>
    <span id="modalReportUserId" class="fw-semibold text-dark"><?php echo e($report->user_id ?? 'N/A'); ?></span>
</div>
...
<div class="modal-footer bg-light border-top rounded-bottom px-4 py-3 d-flex justify-content-between align-items-center">
 ...
 <?php $__currentLoopData = $reports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
  <div id="report-<?php echo e($report->id); ?>" class="card border-0 shadow-sm mb-4">
    <div class="card-body d-flex justify-content-between align-items-center">

      <!-- Status badge -->
      <span class="badge <?php echo e($report->status === 'Forwarded' ? 'bg-success' : 'bg-secondary'); ?>" data-role="status-badge">
        <?php echo e($report->status ?? 'Pending'); ?>
      ...
      <div class="dropdown">
        <button class="btn btn-outline-primary dropdown-toggle" type="button"
                id="forwardDropdown<?php echo e($report->id); ?>" data-bs-toggle="dropdown" aria-expanded="false">
          <i data-lucide="send" class="me-1"></i> Forward To
        </button>
        <ul class="dropdown-menu dropdown-menu-end shadow-sm rounded-3"
            aria-labelledby="forwardDropdown<?php echo e($report->id); ?>">
         <button type="button" 
        class="btn btn-outline-primary dropdown-item" 
        data-user-id="<?php echo e($report->user_id); ?>" 
        onclick="forwardReport(<?php echo e($report->id); ?>, this, 'MDRRMO')">
  MDRRMO
</button>

          <li><a class="dropdown-item" href="javascript:void(0)" onclick="forwardReport(<?php echo e($report->id); ?>, 'Waste Management')">WASTEMANAGEMENT</a></li>
          <li><a class="dropdown-item" href="javascript:void(0)" onclick="forwardReport(<?php echo e($report->id); ?>, 'Water Management')">WATERMANAGEMENT</a></li>
          <li><a class="dropdown-item" href="javascript:void(0)" onclick="forwardReport(<?php echo e($report->id); ?>, 'Fire Department')">Fire Department</a></li>
        </ul>
      </div>
...
function forwardReport(reportId, btn, office) {
  ...
  Swal.fire({
    ...
    fetch("<?php echo e(route('reports.forward')); ?>", {
      method: "POST",
      headers: {
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/bantayan/reports.blade.php ENDPATH**/ ?>