...
<main class="flex-1 flex flex-col overflow-y-auto">
  ...
  <section class="px-6 py-8 space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
      ...
      <div class="relative bg-gradient-to-br from-white to-gray-50 shadow-lg rounded-xl border-l-4 border-<?php echo e($s['color']); ?> p-5 hover:shadow-2xl transition-transform transform hover:-translate-y-1 hover:scale-105 bg-card-texture">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-semibold text-gray-500"><?php echo e($s['label']); ?></p>
            <p class="mt-2 text-3xl font-bold <?php echo e($s['color']); ?>" 
               x-data="{ count:0 }" 
               x-init="let i=0; const interval=setInterval(()=>{ if(i<=<?php echo e($s['value']); ?>) { count=i++; } else clearInterval(interval); },20)">
              <span x-text="count"></span>
            </p>
          </div>
          <div class="w-12 h-12 flex items-center justify-center rounded-full bg-<?php echo e($s['color']); ?>/10">
    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
      <div class="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50">
        ...
        <a href="<?php echo e(route('mdrrmo.reports-bantayan')); ?>" class="px-3 py-1 bg-gray-800 text-white rounded hover:bg-gray-700 transition text-sm">View All</a>
      </div>
        ...
        <table class="w-full min-w-[650px] text-left text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 transition-all">
              <td class="px-4 py-3 font-medium">#<?php echo e($r->id); ?></td>
              <td class="px-4 py-3"><?php echo e(optional($r->created_at)->format('M d, Y')); ?></td>
              <td class="px-4 py-3">
                <?php
                ...
                ?>
                <span class="px-3 py-1 rounded-full text-xs font-semibold <?php echo e($badge); ?>"><?php echo e($r->status ?? 'N/A'); ?></span>
              </td>
              <td class="px-4 py-3">
                <p class="text-gray-700 text-sm truncate" title="<?php echo e($r->description ?? 'No details available'); ?>">
                  <?php echo e($r->description ?? 'No details available'); ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      ...
      <div id="<?php echo e($widget['id']); ?>" class="bg-white shadow-xl rounded-lg border border-gray-200 overflow-hidden hover:shadow-2xl transition-all">
        ...
        <ul class="divide-y divide-gray-100 max-h-64 overflow-y-auto">
          <?php $__empty_1 = true; $__currentLoopData = $widget['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <li class="p-4 hover:bg-gray-50 cursor-pointer transition-all">
            <?php if($widget['id']==='feedback'): ?>
              <p class="text-gray-700 font-medium">"<?php echo e($item->message ?? ''); ?>"</p>
              <p class="text-xs text-gray-500 mt-1">— <?php echo e(optional($item->user)->name ?? 'Anonymous'); ?></p>
            <?php elseif($widget['id']==='alerts'): ?>
              <p class="font-semibold text-red-600">🚨 <?php echo e($item->message ?? ''); ?></p>
            <?php else: ?>
              <p class="font-medium"><?php echo e($item->title ?? ''); ?></p>
              <p class="text-xs text-gray-500"><?php echo e(optional(\Carbon\Carbon::parse($item->when ?? $item->date ?? now()))->format('M d, Y')); ?></p>
            <?php endif; ?>
          </li>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
          <li class="p-4 text-gray-500 text-sm font-medium">No <?php echo e(strtolower($widget['title'])); ?>.</li>
          <?php endif; ?>
        </ul>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php /**PATH C:\Users\ADMIN\my-app\CITIZENENGAGEMENTBANTAYAN\resources\views/dashboard/mdrrmo-bantayan.blade.php ENDPATH**/ ?>