      ...
      <ul class="dropdown-menu dropdown-menu-end shadow-lg p-4 rounded-4" style="width: 280px;" aria-labelledby="adminDropdown">
        <li class="mb-3 d-flex justify-content-between align-items-center">
          ...
          <div class="form-check form-switch m-0">
            <input class="form-check-input" type="checkbox" id="darkModeToggle" title="Toggle dark theme">
          </div>
        </li>
      ...
      <div class="card border-0 shadow-sm mb-4 report-card position-relative hover-glow">
        <div class="card-body d-flex flex-wrap justify-content-between align-items-start gap-3">
          ...
          <div class="text-end" style="z-index: 1050;">
            ...
            <div class="dropup">
              <button class="btn btn-sm bg-light border-0 shadow-sm rounded-3 px-3 py-2 d-flex align-items-center gap-2 dropdown-toggle" 
...
<ul class="dropdown-menu dropdown-menu-end shadow rounded-3" style="z-index:2000;">
  ...
  <div class="modal fade" id="reportModal" tabindex="-1" aria-labelledby="reportModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
      <div class="modal-content shadow-xl border-0 rounded-4" style="backdrop-filter: blur(18px); background: rgba(255,255,255,0.96);">
  ...
  <script>
      ...
      Swal.fire({
        ...
        .then(async res => {
          ...
          if(res.ok && data.success) {
            Swal.fire({ icon:"success", title:"Forwarded!", text:`Report has been forwarded to ${office}.`, timer:2000, showConfirmButton:false });
            const badge = document.querySelector(`#report-${reportId} [data-role="status-badge"]`);
            if(badge) { badge.textContent = `Forwarded to ${office}`; badge.classList.remove("bg-secondary"); badge.classList.add("bg-success"); }
          } else {
            Swal.fire({ icon:"error", title:"Error", text:data.message || "Something went wrong." });
          }
        })
        .catch(err => { Swal.close(); Swal.fire({ icon:"error", title:"Request Failed", text: err.message || "Please try again." }); });
      });
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      modal.addEventListener('show.bs.modal', event => {
        ...
        const userName = trigger.getAttribute('data-user-name') || 'Anonymous';
        const userEmail = trigger.getAttribute('data-user-email') || 'No Email';
        ...
        if(photo && photo.trim() !== '') {
          photoElement.src = photo; photoElement.classList.remove('d-none'); noPhotoText.classList.add('d-none');
        } else { photoElement.classList.add('d-none'); noPhotoText.classList.remove('d-none'); }
    ...
    function printReport() {
      ...
      const content = `<div style="display:flex; align-items:center; gap:20px; margin-bottom:30px;">
        <img src="/images/santafe.png" alt="Santa Fe Logo" style="height:90px;">
        <div><h1 style="margin:0; font-size:26px; color:#0f172a;">Municipality of Santa Fe</h1>
        <h3 style="margin:5px 0 0; font-weight:normal; color:#475569;">Incident Report Summary</h3></div></div>
      ...
      printWindow.document.write(`<!DOCTYPE html><html><head><title>Santa Fe Incident Report</title>
        <style>body { font-family:'Segoe UI',sans-serif; padding:40px; color:#1f2937; background:#fff; } h1,h3 { margin:0; } p { margin:12px 0; line-height:1.6; } hr { border:none; border-top:1px solid #ccc; } @media print { body { margin:0; padding:20px; } }</style>
