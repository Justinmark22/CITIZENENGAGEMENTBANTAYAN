...
<body class="bg-gray-100 font-sans flex min-h-screen" x-data="reportApp()" x-init="fetchReports('Bantayan')">
...
<main class="flex-1 flex flex-col">
  ...
  <header class="bg-white border-b px-6 py-4 flex justify-between items-center">
    <h2 class="text-xl font-semibold text-gray-800">Resolved Reports – Bantayan</h2>
    <div class="flex items-center gap-6">
      ...
      <div class="flex items-center gap-2">
        <span class="text-sm text-gray-600">Admin</span>
         <img src="/images/banmdrrmo.png"  class="h-8 w-8 rounded-full border" alt="User">
      </div>
    </div>
  </header>
...
function reportApp() {
  return {
    ...
    // ✅ Fetch resolved reports specifically for Bantayan
    async fetchReports() {
      try { 
        ...
        // Ensure all reports have announced flag
        ...
        Swal.fire({
          title: 'Error!',
          text: 'Failed to fetch resolved reports for Bantayan.',
          icon: 'error',
    ...
    async postAnnouncement(report, index) {
      try {
        ...
        formData.append('location', 'Bantayan'); // ✅ Hardcoded to Bantayan
        formData.append('report_id', report.id);
        if (formValues.extraPhoto) {
          formData.append('photo', formValues.extraPhoto);
        }

        const response = await fetch('/post-announcement/bantayan', {
          method: 'POST',
          headers: {
