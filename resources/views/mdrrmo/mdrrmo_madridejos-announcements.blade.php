...
<head>
  ...
  <title>MDRRMO MADRIDEJOS – Resolved Reports</title>
  ...
  <header class="bg-white border-b px-6 py-4 flex justify-between items-center">
    ...
    <div class="flex items-center gap-6">
      ...
      <div class="flex items-center gap-2">
        ...
        <img src="/images/mad.png" class="h-8 w-8 rounded-full border" alt="User">
      </div>
    </div>
  </header>
...
function reportApp() {
  return {
    ...
    // ✅ Fetch resolved reports specifically for Madridejos
    async fetchReports() {
      try { 
        const response = await fetch('/api/resolved-reports/madridejos');
        ...
        Swal.fire({
          title: 'Error!',
          text: 'Failed to fetch resolved reports for Madridejos.',
          icon: 'error',
    ...
    async postAnnouncement(report, index) {
      try {
        ...
        formData.append('location', 'Madridejos'); // ✅ Hardcoded to Madridejos
        formData.append('report_id', report.id);
        if (formValues.extraPhoto) {
          formData.append('photo', formValues.extraPhoto);
        }

        const response = await fetch('/post-announcement/madridejos', {
          method: 'POST',
          headers: {
