...
<title>Madridejos Resolved Reports</title>
...
<body class="bg-gray-100 font-sans antialiased flex min-h-screen" x-data="reportApp()" x-init="fetchReports()">
    ...
    <aside class="w-64 bg-gray-900 text-white flex-shrink-0 p-6 flex flex-col">
        <h1 class="text-2xl font-bold mb-8">MDRRMO Madridejos</h1>
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        <h1 class="text-3xl font-bold text-gray-800 mb-8 text-center">Madridejos Resolved Reports</h1>
...
function reportApp() {
    return {
        ...
        async fetchReports() {
            try {
                // ✅ Fetch only Madridejos resolved reports
        ...
        async postAnnouncement(report, index) {
            try {
                const payload = {
                    ...
                    location: 'Madridejos' // enforce Madridejos location
                };
                ...
                this.reports.splice(index, 1); // remove posted report
