  ...
  <div class="p-8 lg:p-12 animate-fadeInUp">
    ...
    <form id="loginForm" method="POST" action="{{ route('login.submit') }}" class="space-y-4">
      ...
      <div id="otpSection" class="hidden">
        <label for="otp" class="block text-gray-300 text-sm mb-1 mt-4">Enter OTP sent to your email</label>
        <input type="text" id="otp" name="otp" maxlength="6"
               class="w-full px-4 py-2 rounded-lg bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500">
      </div>

      <button id="loginBtn" type="submit"
              class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition transform hover:scale-105 mt-4">
        Continue
      </button>
...
<script>
  ...
  loginForm.addEventListener("submit", function(e){
    ...
    // If OTP section is visible, allow submission
    if(!otpSection.classList.contains("hidden")){
      return; // Form submits normally
    }

    // Trigger reCAPTCHA + OTP after 3 failed attempts
    if(attempts >= 3){
      e.preventDefault();
      Swal.fire({
        icon: 'info',
        title: 'ReCAPTCHA & OTP Required',
        text: 'Multiple failed attempts detected. Verify reCAPTCHA to receive OTP.',
        ...
        grecaptcha.ready(function() {
          grecaptcha.execute('{{ env("RECAPTCHA_SITE_KEY") }}', {action: 'login'}).then(function(token) {
            ...
            // Send OTP via backend route (example: /send-otp)
