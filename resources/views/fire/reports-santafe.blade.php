...
<body class="bg-gray-100 font-sans flex min-h-screen" x-data="reportApp()" x-init="fetchReports()">
  ...
  <aside class="fixed md:static inset-y-0 left-0 z-40 w-64 
    ...
    <div class="flex items-center justify-between mb-10">
      <img src="{{ asset('/images/firesantafe.png') }}" alt="Bantayan Fire Logo" class="h-16 w-16 rounded-full object-cover border-2 border-yellow-400 shadow-md">
      <span class="text-2xl font-extrabold tracking-wide drop-shadow-lg">Santa .Fe Fire</span>
    ...
    <nav class="flex flex-col gap-4">
      <div>
        ...
        <a href="{{ route('dashboard.fire-santafe') }}" 
      ...
      <div>
        ...
        <a href="{{ route('fire.reports-santafe') }}" 
      ...
      <div>
        ...
        <a href="{{ route('fire.announcement-santafe') }}" 
           class="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-yellow-400/30 transition-all text-yellow-100">
  ...
  <main class="flex-1 p-6 overflow-y-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Forwarded & Rerouted Reports (Fire Department)</h1>
  ...
  <span class="ml-2">By: {{ $report->user?->name ?? 'N/A' }}</span>
      ...
      @empty
      <p class="text-gray-500 text-center mt-10">No reports found for Waste Management.</p>
      @endforelse
    </div>
