...
<head>
  ...
  <title>Fire Bantayan – Resolved Reports</title>
...
<body class="bg-gray-100 font-sans flex min-h-screen" x-data="reportApp()" x-init="fetchReports()">
  ...
  <aside class="fixed md:static inset-y-0 left-0 z-40 w-64 
    ...
    <div class="flex items-center justify-between mb-10">
      <img src="{{ asset('/images/bantayanfire.png') }}" alt="Bantayan Fire Logo" class="h-16 w-16 rounded-full object-cover border-2 border-yellow-400 shadow-md">
      <span class="text-2xl font-extrabold tracking-wide drop-shadow-lg">Bantayan Fire</span>
    ...
    <nav class="flex flex-col gap-4">
      <div>
        ...
        <a href="{{ route('dashboard.fire-bantayan') }}" 
      ...
      <div>
        ...
        <a href="{{ route('fire.reports-bantayan') }}" 
      ...
      <div>
        ...
        <a href="{{ route('fire.announcement-bantayan') }}" 
           class="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-yellow-400/30 transition-all text-yellow-100">
    ...
    <header class="bg-white border-b px-6 py-4 flex justify-between items-center">
      ...
      <div class="flex items-center gap-6">
        ...
        <div class="flex items-center gap-2">
          ...
          <img src="/images/bantayanfire.png" class="h-8 w-8 rounded-full border" alt="User">
        </div>
      </div>
    </header>
...
function reportApp() {
  return {
    ...
    // ✅ Fetch resolved reports (Bantayan only)
    async fetchReports() {
      try {
        ...
        // Your controller returns an array of reports directly
        this.reports = await response.json();
        this.filteredReports = this.reports;

        console.log('✅ Fetched Bantayan resolved reports:', this.reports);
      } catch (error) {
    ...
    // ✅ Apply filters for search, category, and date
    applyFilters() {
    ...
    // ✅ Post a resolved report as an announcement
    async postAnnouncement(report, index) {
      try {
        const { value: formValues } = await Swal.fire({
          title: 'Post Announcement',
          html: `
            <p class="text-sm text-gray-600 mb-2">Add more details for this announcement:</p>
            <input id="swal-extra-title" class="swal2-input" placeholder="Additional Title">
        ...
        formData.append('location', 'Bantayan'); // ✅ Fixed for Bantayan
