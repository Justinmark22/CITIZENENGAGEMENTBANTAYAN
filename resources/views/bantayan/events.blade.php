...
<head>
  <meta charset="UTF-8">
  <title>Bantayan Admin - Events</title>
          ...
          <div class="card-body d-flex flex-column justify-content-between">
            ...
            <div class="d-flex justify-content-between align-items-center mt-auto pt-2 border-top">
              ...
              <div class="d-flex gap-2 mt-2">
                <a href="{{ route('bantayan.events_edit', $event->id) }}" class="btn btn-sm btn-outline-primary">
                  <i class="bi bi-pencil"></i> Edit
                </a>
                <form action="{{ route('bantayan.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Delete this event?');">
                  @csrf
                  @method('DELETE')
...
<div class="modal fade" id="forwardModal{{ $event->id }}" tabindex="-1" aria-labelledby="forwardModalLabel{{ $event->id }}" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <form method="POST" action="{{ route('bantayan.events.forward', $event->id) }}">
      @csrf
